...
- **Live team-activity view (README "The team, live").** A new host half
  ...
  floater: a bottom-right pill, expanded into a live panel showing each
  ...
  on `webServer`). All panel colors are `--dsw-alias` tokens, so it follows the
  shell's own light/dark theme. Design adapted from
